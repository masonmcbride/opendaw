# uv run --python 3.14 --with numpy --with dearpygui python block_daw_osc_focus.py
...
def make_demo_daw() -> DAW:
    ...
    kick = Instrument([Oscillator("sine", 1.0)], decay=0.10, pitch_drop=24.0, click=0.20, drive=0.15)
...
def lfo_array(shape: str, t: np.ndarray) -> np.ndarray:
    x = np.mod(t, 1.0)
    return (np.sin(2 * np.pi * x) if shape == "sine" else np.where(x < 0.5, 1.0, -1.0) if shape == "square" else 2.0 * x - 1.0 if shape == "saw" else 2.0 * np.abs(2.0 * x - 1.0) - 1.0).astype(np.float32)
...
def one_pole_lowpass(x: np.ndarray, cutoff: np.ndarray) -> np.ndarray:
    ...
    for i, xi in enumerate(x):
        c = np.clip(float(cutoff[i]), 20.0, SR * 0.45)
        a = np.float32(1.0 - math.exp(-2.0 * math.pi * c / SR))
        prev = prev + a * (xi - prev)
        y[i] = prev
    return y

def filter_signal(x: np.ndarray, cutoff: np.ndarray, resonance: float, mode: str) -> np.ndarray:
    if mode == "none": return x
    boosted = x + np.float32(resonance) * (x - one_pole_lowpass(x, cutoff))
    return one_pole_lowpass(boosted.astype(np.float32), cutoff)

def pitch_curve(base: float, t: np.ndarray, total: float, pitch_drop: float) -> np.ndarray:
    if pitch_drop <= 0.0: return np.full(len(t), np.float32(base), np.float32)
    frac = np.minimum(1.0, t / max(min(0.08, total), 1e-9))
    return (base * 2.0 ** ((pitch_drop * (1.0 - frac)) / 12.0)).astype(np.float32)

def osc_bank(inst: Instrument, hz: np.ndarray, lfo: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    mix = np.zeros(len(hz), np.float32)
    for osc in inst.oscillators:
        ratio = np.float32(2.0 ** ((osc.detune_semitones + osc.detune_cents / 100.0 + osc.wobble_to_pitch * inst.wobble_depth * lfo) / 12.0))
        phase = np.cumsum(hz * ratio / SR, dtype=np.float32) + np.float32(osc.phase_offset)
        mix += np.float32(osc.gain) * osc_array(osc.waveform, phase, rng)
    if inst.sub_gain:
        sub = np.sin(2 * np.pi * np.mod(np.cumsum((hz * 0.5) / SR, dtype=np.float32), 1.0)).astype(np.float32)
        mix += np.float32(inst.sub_gain) * sub
    return mix
...
def render_note(song: Song, inst: Instrument, note: Note, rng: np.random.Generator) -> np.ndarray:
    ...
    env = adsr_array(inst, t, hold)
    wobble = lfo_array(inst.wobble_shape, t * np.float32(inst.wobble_rate)) if inst.wobble_rate > 0 and inst.wobble_depth > 0 else np.zeros(len(t), np.float32)
    hz = pitch_curve(midi_to_hz(note.pitch), t, total, inst.pitch_drop)
    tonal = osc_bank(inst, hz, wobble, rng)
    ...
    raw = tonal + noise + click
    cutoff = np.full(len(t), np.float32(inst.filter_cutoff), np.float32)
    if inst.filter_env_amount:
        cutoff *= np.float32(2.0 ** (inst.filter_env_amount * env))
    if inst.wobble_depth:
        cutoff *= np.float32(2.0 ** (inst.wobble_depth * wobble))
    shaped = filter_signal(raw.astype(np.float32), cutoff, inst.filter_resonance, inst.filter_mode)
    if inst.drive:
        shaped = np.tanh((1.0 + np.float32(inst.drive) * 4.0) * shaped).astype(np.float32)
    return (np.float32(inst.gain * note.velocity) * env * shaped).astype(np.float32)
...
def note_key(inst: Instrument, note: Note):
    osc_key = tuple((o.waveform, o.gain, o.detune_semitones, o.detune_cents, o.phase_offset, o.wobble_to_pitch) for o in inst.oscillators)
    return (osc_key, inst.attack, inst.decay, inst.sustain, inst.release, inst.pitch_drop, inst.noise, inst.click, inst.sub_gain, inst.filter_mode, inst.filter_cutoff, inst.filter_resonance, inst.filter_env_amount, inst.wobble_rate, inst.wobble_depth, inst.wobble_shape, inst.drive, inst.gain, note.pitch, note.length_ticks, note.velocity)
...
def accumulate_events(song: Song, events: list[tuple[int, Instrument, Note]] | None = None) -> np.ndarray:
    ...
    for start, inst, note in note_events(song) if events is None else events:
        deterministic = inst.noise == 0.0 and all(o.waveform != "noise" for o in inst.oscillators)
        key = note_key(inst, note)
...
def master_signal(x: np.ndarray, mode: str = "normalize") -> np.ndarray:
    ...
    if mode == "bass": return np.tanh(2.2 * np.tanh(1.35 * x)).astype(np.float32)
...
def heat(v: float, lo: float, hi: float):
    t = max(0.0, min(1.0, (float(v) - lo) / max(hi - lo, 1e-9)))
    return (int(70 + 185 * t), int(220 - 90 * abs(t - 0.5) * 2), int(80 + 120 * (1 - t)), 255)

def inst_numeric(inst: Instrument):
    return [("attack", inst.attack, 0, 1), ("decay", inst.decay, 0, 1), ("sustain", inst.sustain, 0, 1), ("release", inst.release, 0, 2), ("pitch_drop", inst.pitch_drop, 0, 24), ("noise", inst.noise, 0, 1), ("click", inst.click, 0, 1), ("sub", inst.sub_gain, 0, 1), ("cutoff", inst.filter_cutoff, 20, 4000), ("res", inst.filter_resonance, 0, 1), ("env", inst.filter_env_amount, 0, 2), ("wob_rate", inst.wobble_rate, 0, 12), ("wob_depth", inst.wobble_depth, 0, 1), ("drive", inst.drive, 0, 1), ("gain", inst.gain, 0, 1), ("pan", inst.pan, -1, 1)]

def plot_line(label: str, xs, ys, h=140):
    with dpg.plot(label=label, height=h, width=245, no_mouse_pos=True):
        dpg.add_plot_axis(dpg.mvXAxis, no_tick_labels=True, no_gridlines=True)
        yaxis = dpg.add_plot_axis(dpg.mvYAxis, no_tick_labels=True, no_gridlines=True)
        dpg.add_line_series(list(xs), list(ys), parent=yaxis)

def draw_matrix(label: str, matrix: np.ndarray, w=245, h=140):
    dpg.add_text(label, color=TEXT)
    tag = dpg.add_drawlist(width=w, height=h)
    a = matrix.astype(np.float32); a = (a - a.min()) / max(float(a.max() - a.min()), 1e-9)
    rows, cols = a.shape; cw, ch = w / cols, h / rows
    for r in range(rows):
        for c in range(cols):
            dpg.draw_rectangle((c*cw, r*ch), ((c+1)*cw+1, (r+1)*ch+1), fill=heat(a[r, c], 0, 1), color=heat(a[r, c], 0, 1), parent=tag)

def show_instrument_window(daw: DAW, i: int):
    track, inst = daw.song.tracks[i], daw.song.tracks[i].instrument
    note = Note(0, ticks_per_beat(daw.song), 48, 1.0)
    y = render_note(daw.song, inst, note, np.random.default_rng(0))[:SR]
    t = np.linspace(0, len(y) / SR, len(y), endpoint=False); small = slice(None, None, max(1, len(y)//512))
    spec = np.abs(np.fft.rfft(y[:min(len(y), 8192)])); freqs = np.fft.rfftfreq(min(len(y), 8192), 1/SR)
    env = adsr_array(inst, t.astype(np.float32), ticks_to_seconds(daw.song, note.length_ticks))
    lfo = lfo_array(inst.wobble_shape, (t * inst.wobble_rate).astype(np.float32)) if inst.wobble_rate and inst.wobble_depth else np.zeros_like(t, np.float32)
    cutoff = inst.filter_cutoff * (2.0 ** (inst.filter_env_amount * env)) * (2.0 ** (inst.wobble_depth * lfo))
    frames = [np.abs(np.fft.rfft(y[k:k+512] * np.hanning(min(512, len(y)-k)), 512))[:64] for k in range(0, max(1, len(y)-512), 256)]
    if dpg.does_item_exist("instrument_panel"): dpg.delete_item("instrument_panel")
    with dpg.window(tag="instrument_panel", label=f"Instrument Dashboard · {track.name}", width=860, height=780, pos=(700, 60)):
        dpg.add_text(f"focused track {i}: {track.name} · {len(inst.oscillators)} osc · filter={inst.filter_mode}", color=TEXT)
        dpg.add_separator()
        with dpg.table(header_row=False, borders_innerH=True, borders_innerV=True, policy=dpg.mvTable_SizingFixedFit):
            for _ in range(4): dpg.add_table_column(width_fixed=True)
            for row in range(0, len(inst_numeric(inst)), 4):
                with dpg.table_row():
                    for name, v, lo, hi in inst_numeric(inst)[row:row+4]: dpg.add_text(f"{name}: {v:.3g}", color=heat(v, lo, hi))
        dpg.add_separator()
        with dpg.table(header_row=False, borders_innerV=True, policy=dpg.mvTable_SizingStretchSame):
            dpg.add_table_column(); dpg.add_table_column(); dpg.add_table_column()
            panels = [("waveform", t[small], y[small]), ("envelope", t[small], env[small]), ("spectrum", freqs[:512], np.log1p(spec[:512])), ("pitch Hz", t[small], pitch_curve(midi_to_hz(48), t.astype(np.float32), t[-1], inst.pitch_drop)[small]), ("cutoff", t[small], cutoff[small]), ("LFO", t[small], lfo[small])]
            for k in range(0, len(panels), 3):
                with dpg.table_row():
                    for label, xs, ys in panels[k:k+3]:
                        with dpg.group(): plot_line(label, xs, ys)
            with dpg.table_row():
                with dpg.group(): draw_matrix("spectrogram", np.stack(frames).T if frames else np.zeros((64, 1)))
                with dpg.group(): plot_line("osc gains", range(len(inst.oscillators)), [o.gain for o in inst.oscillators])
                with dpg.group(): plot_line("parameter heat", range(len(inst_numeric(inst))), [(v-lo)/max(hi-lo, 1e-9) for _, v, lo, hi in inst_numeric(inst)])
        dpg.add_separator(); dpg.add_text("oscillators: " + ", ".join(f"{o.waveform}:{o.gain:g}@{o.detune_semitones:g}st/{o.detune_cents:g}c" for o in inst.oscillators), color=TEXT_SOFT)

def focus_track(daw: DAW, i: int):
    daw.ui.focused_track, daw.ui.focused_block, daw.dirty = i, None, True
    show_instrument_window(daw, i)
...
def rebuild_rows(daw: DAW) -> None:
    ...
    for i, track in enumerate(daw.song.tracks):
        with dpg.group(horizontal=True, parent=ROWS):
            ...
            dpg.add_button(label=track.name, width=150, height=22, callback=lambda s, a, u: focus_track(*u), user_data=(daw, i))
